...
**素材を手渡しでもらう必要はありません。ビルドに要るものは全部リポジトリに入っています。**

```bash
git clone --depth 1 https://github.com/KMC48/valentine-bokumetsu.git
cd valentine-bokumetsu/app
npm ci
```

> `--depth 1` を付けると、過去の履歴を飛ばして最新だけ取ってきます。
> 差し替えた古い画像が履歴に残っているため、全部取ると 140MB ほどになります。
> **iOSのビルドに履歴は不要です。**

#### リポジトリに入っていないもの

いずれも**ビルドには使いません。**絵を作り直したくなったときだけ必要になります。

| | 中身 | なぜ入れていないか |
|---|---|---|
| `画像素材/使用・未使用/` | 納品された生のPNG 166枚 | 加工前の原本。容量が大きい |
| `app/素材作業用/` | BGMのWAV原本、作業用ファイル | 配信はMP3。WAVは37MBある |
| `確認用スクショ/` | 動作確認用の画面キャプチャ | 手元で見るためのもの |

**ゲームが実際に読む画像・音声は `app/public/assets/` に入っていて、これはコミット済みです。**

> **確認済み**：まっさらな場所にクローンして `npm ci` → `npm run build` を流し、
> 24MB の `dist/` ができてテスト88件が通ることを確かめてあります。
